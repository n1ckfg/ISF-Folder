...
void GetCameraRay( const in vec3 vPos, const in vec3 vForwards, const in vec3 vWorldUp, out C_Ray ray)
{
	...
	#endif
	vec2 vUV = ( vPixelCoord / RENDERSIZE.xy );
	vec2 vViewCoord = vUV * 2.0 - 1.0;

	vViewCoord *= 0.75;
       
	float fRatio = RENDERSIZE.x / RENDERSIZE.y;
...
void main( void )
{
	...
	vec2 vCenterPosition = (0.5 - ( gl_FragCoord.xy / RENDERSIZE )) * surfaceSize + vv_FragNormCoord;
	...
	#ifdef ENABLE_MONTE_CARLO                              
	vec3 cPrev = IMG_NORM_PIXEL(backbuffer,gl_FragCoord.xy / RENDERSIZE).xyz;
