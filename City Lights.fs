...
void main() {
	...
	//	next three passes are doing an erode.  the first and third are doing horizontal sampling, the second is doing vert sampling
		...
		for (float i=0.; i<=localBlurRadius; ++i)	{
			if (PASSINDEX==1)	{
				tmpCoord = vec2(clamp(vv_FragNormCoord.x+(i*pixelWidth.x), 0., 1.), vv_FragNormCoord.y);
				sampleColorRGB = IMG_NORM_PIXEL(edges, tmpCoord);
			}
			else if (PASSINDEX==2)	{
				tmpCoord = vec2(vv_FragNormCoord.x, clamp(vv_FragNormCoord.y+(i*pixelWidth.y), 0., 1.));
	...
	//	...writes into the full-size
		...
		vec4		originalImg = IMG_NORM_PIXEL(edges,vv_FragNormCoord);
		if (blurLevel == 0)
